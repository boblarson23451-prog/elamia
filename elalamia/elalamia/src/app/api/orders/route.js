...
export async function POST(req) {
  ...
  const { fullName, phone, wilaya, address, paymentMethod } = await req.json();
  ...
  const cartItems = db
    .prepare(
      `SELECT ci.quantity, p.id as product_id, p.name_ar, p.name_fr, p.price, p.stock
  ...
  const createOrder = db.transaction(() => {
    ...
    const insertItem = db.prepare(
      `INSERT INTO order_items (order_id, product_id, name_ar, name_fr, price, quantity)
       VALUES (?, ?, ?, ?, ?, ?)`
    );
    for (const item of cartItems) {
      insertItem.run(orderId, item.product_id, item.name_ar, item.name_fr, item.price, item.quantity);
