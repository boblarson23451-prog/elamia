...
export default function CheckoutPage() {
  ...
  const [gateways, setGateways] = useState({ chargily: false, sofizpay: false });
  ...
  const submit = async (e) => {
    ...
    if (res.ok) {
      ...
      if (data.checkoutError) {
        router.push(`/account/orders/${data.orderId}?payment_error=1`);
        return;
      }
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <label
                ...
                style={{
                  borderColor: form.paymentMethod === "cod" ? "var(--color-brand)" : "var(--color-line)",
              ...
              {gateways.chargily && (
                <label
                  className="flex items-center gap-2 border rounded-lg px-3 py-2.5 text-sm cursor-pointer"
                  style={{
                    borderColor: form.paymentMethod === "chargily" ? "var(--color-brand)" : "var(--color-line)",
                    background: "var(--color-paper)",
                  }}
                >
                  <input
                    type="radio"
                    name="pm"
                    checked={form.paymentMethod === "chargily"}
                    onChange={() => update("paymentMethod", "chargily")}
                  />
                  {t("payChargily")}
                </label>
              )}
              ...
              {gateways.sofizpay && (
                <label
                  className="flex items-center gap-2 border rounded-lg px-3 py-2.5 text-sm cursor-pointer"
                  style={{
                    borderColor: form.paymentMethod === "sofizpay" ? "var(--color-brand)" : "var(--color-line)",
                    background: "var(--color-paper)",
                  }}
                >
                  <input
                    type="radio"
                    name="pm"
                    checked={form.paymentMethod === "sofizpay"}
                    onChange={() => update("paymentMethod", "sofizpay")}
                  />
                  {t("paySofizpay")}
                </label>
              )}

              {!gateways.chargily && !gateways.sofizpay && (
                <label
